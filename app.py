...
def fetch_yt_video(link):
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            return info.get('title', 'Resume Writing Tips')
    except Exception:
...
def insert_data(name, email, res_score, timestamp, no_of_pages, reco_field, cand_level, skills, recommended_skills, courses):
    ...
    # Explicitly defining columns prevents "Column count doesn't match" errors
    insert_sql = f"""
    ...
    # connection.commit() is handled by autocommit=True now    
...
def run():
    ...
    link = '[©Developed by Mani Ratan](https://www.linkedin.com/in/mani-ratan-159963289)'
    ...
    if choice == 'User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                # --- FIX 1: Allow User to Correct the AI ---
                st.subheader("**Verify Your Basic info**")
                
                # Use .get() to avoid errors if a field is missing
                extracted_name = resume_data.get('name', 'User')
                extracted_email = resume_data.get('email', 'Not Found')
                extracted_mobile = resume_data.get('mobile_number', 'Not Found')
                extracted_pages = resume_data.get('no_of_pages', 0)

                # Editable fields so the user can fix "Great Lakes" to "Allison Beer"
                name = st.text_input('Name:', value=extracted_name)
                email = st.text_input('Email:', value=extracted_email)
                mobile = st.text_input('Contact:', value=extracted_mobile)
                
                st.write(f"**Resume pages:** {extracted_pages}")

                # --- FIX 2: Better Level Logic ---
                cand_level = ''
                if extracted_pages == 1:
                    cand_level = "Fresher"
                    st.markdown('''<h4 style='color: #d73b5c;'>Level: Fresher</h4>''', unsafe_allow_html=True)
                elif extracted_pages == 2:
                    cand_level = "Intermediate"
                    st.markdown('''<h4 style='color: #1ed760;'>Level: Intermediate</h4>''', unsafe_allow_html=True)
                else:
                    cand_level = "Experienced"
                    st.markdown('''<h4 style='color: #fba171;'>Level: Experienced</h4>''', unsafe_allow_html=True)
                ...
                if 'Objective' in resume_text:
                    ...
                    st.markdown('''<h5 style='text-align: left; color: #FFB020;'>[-] Please add your career objective, it will give your career intention to the Recruiters.</h5>''', unsafe_allow_html=True)

                if 'Declaration' in resume_text:
                    resume_score = resume_score + 20
                    st.markdown('''<h5 style='text-align: left; color: #1ed760;'>[+] Awesome! You have added Declaration</h5>''', unsafe_allow_html=True)
                else:
                    st.markdown('''<h5 style='text-align: left; color: #FFB020;'>[-] Please add Declaration. It will give the assurance that everything written on your resume is true and fully acknowledged by you</h5>''', unsafe_allow_html=True)
                ...
                if 'Hobbies' in resume_text or 'Interests' in resume_text:
                    ...
                    st.markdown('''<h5 style='text-align: left; color: #FFB020;'>[-] Please add Hobbies. It will show your personality to the Recruiters and give the assurance that you are fit for this role or not.</h5>''', unsafe_allow_html=True)
                ...
                if 'Achievements' in resume_text:
                    ...
                    st.markdown('''<h5 style='text-align: left; color: #FFB020;'>[-] Please add Achievements. It will show that you are capable for the required position.</h5>''', unsafe_allow_html=True)
        ...
        if st.button('Login'):
            if ad_user == st.secrets["admin_user"] and ad_password == st.secrets["admin_password"]:
                st.success("Welcome Mr. Mani Ratan !")
                # Display Data
                ...
                # --- FIX START: Convert Bytes to Strings ---
                # This decodes the BLOB data from the database so Plotly can read it
